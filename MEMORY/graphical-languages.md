...
| **3c. FBD authoring** | Drag blocks, draw wires (drag-from-output-pin-to-input-pin), pick FB types from the toolbar. Pure SVG + pointer events — no react-flow dependency. | done 2026-05-16 | Build a 3-block pipeline entirely with the mouse: "+ Block" picks the FB, drag headers to lay it out, drag output pins to input pins to wire them, edit details inline. | ✅ shipped |
| **4. FBD position persistence (= CFC)** | Save dragged positions to JSON. Already comes free with 3c — `FbdPosition` is on every `FbdBlock` and survives reload. We don't call this a separate language ("CFC") because the schema is identical to FBD. | done 2026-05-16 | Drag a block, save, reload — block stays put. | ✅ shipped |
...
| **5b. SFC renderer + editor** | IEC-style vertical-flow rendering: double-frame initial step, thick horizontal transition bars crossing the flow line, **action boxes as separate two-column rectangles to the right of each step** (qualifier letter + body). Full authoring: add/remove/rename step, mark initial, add/edit/delete action, add/edit/reorder/delete transition, all via click-to-select + bottom detail bar. | done 2026-05-16 | `batch_sequence.sfc.json` renders + edits as a 5-step state machine; new steps and transitions can be added entirely from the UI. | ✅ shipped |
